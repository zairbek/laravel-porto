...
class Task
{
    ...
    private function resolveClass($class)
    {
        ...
        if ($this->needsParsing($class)) {
            ...
            $class = $classFullName = \Task::buildClassFullName($containerName, $className);

            \Task::verifyClassExist($classFullName);
        } else {
    ...
    {
        return 'App\Containers\\' . $containerName . '\\Tasks\\' . $className;
    }
